<template>
  <div>
    ...
    <el-card>
      <!-- 提示 -->
      <el-alert title="添加商品信息" type="info" center class="el-icon-info" :closable="false"></el-alert>
  ...
  methods: {
    ...
    async getCateList () {
      ...
      if (res.meta.status !== 200) {
        return this.$notify.error({ title: '获取商品分类数据失败！', message: '请重新登录后再试' })
      }
    ...
    beforeTabLeave (activeName, oldActiveName) {
      if (oldActiveName === '0' && this.addForm.goods_cat.length !== 3) {
        this.$notify.error({ title: '无法进入' + (activeName === '1' ? '商品参数' : (activeName === '2' ? '商品属性' : (activeName === '3' ? '商品图片' : (activeName === '4' ? '商品内容' : activeName ? '' : '')))) + '选项', message: '请先选择商品分类' })
    ...
    async tabClicked () {
      ...
      if (this.activeIndex === '1') {
        ...
        if (res.meta.status !== 200) {
          return this.$notify.error({ title: '获取动态参数列表失败！' })
        }
        ...
        if (res.meta.status !== 200) {
          return this.$notify.error({ title: '获取静态属性失败！' })
        }
    ...
    add () {
      this.$refs.addFormRef.validate(async valid => {
        ...
        this.manyTableData.forEach(item => {
          const newInfo = { attr_id: item.att_id, attr_value: item.attr_vals.join(' ') }
        ...
        if (res.meta.status !== 201) return this.$notify.error({ title: '添加商品失败' })
      })
      this.$notify.success({ title: '添加商品成功' })
